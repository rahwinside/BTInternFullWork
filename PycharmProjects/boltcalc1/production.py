...
class UX(QDialog, newui.Ui_bolt_ui):
    def __init__(self):
        QDialog.__init__(self)
        newui.Ui_bolt_ui.__init__(self)
        self.setupUi(self)
        ...
        self.thicklabel.hide()
        self.label_41.hide()
        self.label_42.hide()
        self.label_43.hide()
        self.label_44.hide()
        self.sizelineEdit.hide()
        self.pitchlineEdit.hide()
        self.thicklineEdit.hide()
        self.rBlock_2.hide()
        self.sBlock_2.hide()
        self.tBlock_2.hide()
        self.uBlock_2.hide()
        ...
        self.sizelineEdit.editingFinished.connect(self.setBoltSize)
    ...
    def saveInputData(self):
        try:
            ...
            if self.typeCombo.currentText() == 'Non-Standard/Stud ':
            ...
            if self.typeCombo.currentText() == 'Non-Standard/Stud ':
                data.append(('height', float(self.thicklineEdit.text())))

                dimen = [float(self.rBlock_2.text()),
                         float(self.sBlock_2.text()),
                         float(self.tBlock_2.text()),
                         float(self.uBlock_2.text())]

                data.append(('dw', 2 * min(dimen)))
                data.append(('pitch', float(self.pitchlineEdit.text())))
                ...
                for item in bolttab:
                    if str(item['size']) == str(size):
                        ...
                        print(self.typeCombo.currentText().split())

                        if 'Countersunk' in self.typeCombo.currentText().split():
                            csa = math.pi * math.sqrt(2) * (math.pow(float(item['dw'])/2, 2) -
                                                            math.pow(float(item['dw'])/2 - float(item['k']), 2))
                            dw = 8 / (3 * math.pi) * csa
                            data.append(('dw', dw))
                        else:
    ...
    def saveClampData(self):
        global clamped_data, bolt_data, threaded_data, bolt_size
        clamped_data.clear()
        ...
        try:
            for i in range(self.scrollLayout.count()):
                ...
                if x.washerComboBox.currentText() == 'Rectangular':
                    net_thickness += float(x.rectThickness.text())
                else:
                    net_thickness += float(x.thicklineEdit.text())
            ...
            for i in range(self.scrollLayout.count()):
                ...
                print(x.washerComboBox.currentText())
                if x.washerComboBox.currentText() == 'Rectangular':
                    dimen = [float(x.insdialineEdit.text()),
                             float(x.outdialineEdit.text()),
                             float(x.thicklineEdit.text()),
                             float(x.uEdit.text())]
                    ...
                    l.append(('thick', float(x.rectThickness.text())))

                    if x.typeComboBox.currentText() == 'Circular hole':
                        l.append(('insdia', float(x.holeDia.text())))

                    elif x.typeComboBox.currentText() == 'Oblong hole':
                        l.append(('d', float(x.holeDia.text())))
                        l.append(('l', float(x.holeLength.text())))

                        angle = (180) * math.pi / 180

                        bolt_std = self.typeCombo.currentText()
                        nut_std = self.partCombo.currentText()
                        d = float(x.holeDia.text())
                        if not bolt_std == 'Non-Standard/Stud' and nut_std == 'Nut':
                            if cumulative_thickness < net_thickness:
                                D = bolt_data['dw'] + 2 * cumulative_thickness * math.tan(angle)
                                deq = math.sqrt(4 * D * d / math.pi)
                            else:
                                tn = net_thickness - cumulative_thickness - float(x.thicklineEdit.text())
                                D = threaded_data['dw'] + 2 * tn * math.tan(angle)
                                deq = math.sqrt(4 * D * d / math.pi)

                        elif bolt_std == 'Non-Standard/Stud' and nut_std == 'Nut':
                            if cumulative_thickness < net_thickness:
                                D = 2 * (0.8 * float(bolt_size) + cumulative_thickness) * math.tan(angle)
                                deq = math.sqrt(4 * D * d / math.pi)
                            else:
                                tn = net_thickness - cumulative_thickness - float(x.thicklineEdit.text())
                                D = threaded_data['dw'] + 2 * tn * math.tan(angle)
                        ...
                        elif not bolt_std == 'Non-Standard/Stud' and not nut_std == 'Nut':
                            if cumulative_thickness < net_thickness:
                                D = bolt_data['dw'] + 2 * cumulative_thickness * math.tan(angle)
                                deq = math.sqrt(4 * D * d / math.pi)
                            else:
                                tn = net_thickness - cumulative_thickness - float(x.thicklineEdit.text())
                                D = 2 * (0.8 * float(bolt_size) + tn) * math.tan(angle)
                                deq = math.sqrt(4 * D * d / math.pi)
                ...
                cumulative_thickness += float(x.thicklineEdit.text())
            print(clamped_data)
        except:
    ...
    def populateSizeList(self):
        print(self.typeCombo.currentText())
        if self.typeCombo.currentText() == 'Non-Standard/Stud ':
            print('hi')
            self.sizeLabel.show()
            self.pitchlabel.show()
            self.thicklabel.show()
            self.label_41.show()
            self.label_42.show()
            self.label_43.show()
            self.label_44.show()
            self.sizelineEdit.show()
            self.pitchlineEdit.show()
            self.thicklineEdit.show()
            self.rBlock_2.show()
            self.sBlock_2.show()
            self.tBlock_2.show()
            self.uBlock_2.show()
    ...
    def setBoltSize(self):
        ...
        if not (self.sizeCombo.currentText() == ''):
            if (self.typeCombo.currentText() == 'Non-Standard/Stud '):
                if not (self.sizelineEdit.text() == ''):
                    bolt_size = float(self.sizelineEdit.text().lstrip('M'))
            else:
    ...
    def fillSpreadsheet(self):
        try:
            ...
            # write part nature to sheet
            if 'Non-Standard/Stud' in self.typeCombo.currentText():
                ws['H6'] = 'non standard'
            else:
                ws['H6'] = 'standard'
            if 'Block' in self.partCombo.currentText():
                ws['H7'] = 'non standard'
            else:
                ws['H7'] = 'standard'
            ...
            ws['A67'] = 'Head/Part1'
            ws['B67'] = '=MIN(O7,Q16)'
            ws['D67'] = '=IF(B67>C67,"Pass","Fail")'
            nClamped = int(self.nClampedPartsText.text())

            for i in range(nClamped):
                ws['A'+str(i+68)] = 'Part'+str(i+1)+'/Part'+str(i+2)
                ws['B'+str(i+68)] = '=MIN(Q'+str(i+16)+',Q'+str(i+17)+')'
                ws['D'+str(i+68)] = '=IF(B'+str(i+68)+'>C'+str(i+68)+',"Pass","Fail")'

            ws['A' + str(i + 68)] = 'Part' + str(i + 1) + '/Threaded Part'
            ws['B' + str(i + 68)] = '=MIN(Q' + str(i + 16) + ',O35)'
            ws['D' + str(i + 68)] = '=IF(B' + str(i + 68) + '>C' + str(i + 68) + ',"Pass","Fail")'
