...
def loadcase(wkbook):
    ...
    try:
        ...
        try:
            ...
            print(sheet.cell_value(2, sheet.ncols-1))
            print(highestIDVal)
        except:
            pass
...
def loadstep(wkbook):
    ...
    nLCase = int(input("Enter no. of load cases:"))
    nSPC = input("Enter no. of SPCs:")
    ...
    tcl_file_name = 'new.tcl'
    new_tcl = open(tcl_file_name, 'a')

    pretensionEnabled = False
    if (1 in pretension):
        pretensionEnabled = True
        pretspcid = input("Enter SPC ID for Pretension:")
        pretloadid = int(pretsheet.cell_value(2, pretsheet.ncols-1)) #input("Enter Pret ID for Pretension:")
    ...
    for i in range(0, len(loadstep_matrix)):
        ...
        if (loadstep_sheet.cell_value(i + 2, 1) == 'linear static'):
            ...
            try:
                ...
                print("error: Load+SPC count >2! in step " + id + "\n Loadstep TCl !Gen")
                exit(1)
...
wkbook = input("Enter path to workbook:")
file = open(wkbook, 'r')
