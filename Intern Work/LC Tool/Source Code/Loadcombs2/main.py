...
class Ui_load(object):
    def setupUi(self, load):
        ...
        self.aboutButton = QtWidgets.QPushButton(load)
        self.aboutButton.setObjectName("aboutButton")
        self.gridLayout.addWidget(self.aboutButton, 10, 0, 1, 1)
    ...
    def retranslateUi(self, load):
        ...
        load.setWindowTitle(_translate("load", "Load Step Generator for HyperMesh"))
        self.continueLabel.setText(_translate("load",
                                              "<html><head/><body><p align=\"center\">Fill the generated Excel sheet and click &quot;Continue&quot; to generate TCL</p></body></html>"))
        ...
        self.label_6.setText(_translate("load",
                                        "<html><head/><body><p><span style=' font-weight:600; color:#e70000;'>Check the following in HyperMesh before using the tool:</span></p><p><span style=' font-weight:600;'>1. TCL script to extract SPC and LOAD details is run in HyperMesh.</span></p><p><span style=' font-style:italic;'>---the extracted info will be stored in D:/collector.txt;</span></p><p><span style=' font-style:italic;'>---will be automatically retrieved by this tool.</span></p><p><span style=' font-weight:600;'>2. All load collectors and load steps don't have spaces in their names (underscores are allowed).</span></p><p align='center'><span style=' font-weight:600; font-style:italic; color:#de0003;'>Error/Activity Log</span></p></body></html>"))
        ...
        self.aboutButton.setText(_translate("load", "About"))
...
class UX(QDialog, Ui_load):
    ...
    def about(self):
        ...
        msg.setText(
            '<html><head/><body><p><span style=" font-weight:600;">Load Step Generator for HyperMesh</span></p><p><span style=" font-weight:600;">Version 1.0.2 </span>Stable</p><p>Build 20190717<br/></p><p><span style=" font-weight:600;">Tool compatibility:</span></p><p>Tested to work on HyperMesh v2014.x and v2017.2</p><p><span style=" font-weight:600;">Usage:</span></p><p>1. Generation of LOADADD entities</p><p>2. Load step generation - for linear/non-linear analysis with and without Pretension</p></body></html>')
    ...
    def gen_excel(self):
        ...
        try:
            try:
                raw_data = open("D:/collector.txt", 'r')
            except:
            ...
            ldstep.write(2, i+3, "DON'T FILL ANYTHING IN THIS ROW, LEFT FOR SPC IDs; MAKE SELECTIONS FROM THE NEXT ROW", cell_format)
            wkbk.close()
            ...
            self.errorLabel.setText(
                self.errorLabel.text() + 'Error: Workbook is possibly open!\nPlease close the workbook and try again!\n')
            print(e)
    ...
    def spcAdd(self, wkbook, id):
        ...
        for item in selSPCNoDupl:
            ...
            new_tcl.write(
                '*dictionaryload loadcols 2 "' + hyper_path + '/templates/feoutput/optistruct/optistruct" "SPCADD"\n')
            ...
            text += '\n'
            new_tcl.write(text)
            new_tcl.write('*attributeupdateentityidarray loadcols ' + str(id) + ' 370 1 2 0 loadcols 1 ' + str(
                item.count(1)) + '\n')
    ...
    def loadcase(self, wkbook):
        ...
        try:
            ...
            self.errorLabel.setText("TCL generated for LOADADD!\n")
            new_tcl.close()
            global_dict = dict(l1)
            # print(global_dict)
            ...
            msg.setText("Failed to generate TCL!")
    ...
    def loadstep(self, wkbook):
        ...
        try:
            ...
            i = 3
            while (i < 16384):
                if not (loadstep_sheet.cell_value(1, i) in [0, '']):
                    nls_attributes += 1
                    i += 1
                else:
                    break
            ...
            loadstep_matrix = []
            print(nLoadsteps, nls_attributes)
            for p in range(3, nLoadsteps + 3):
                x = []
                for q in range(3, nls_attributes + 3):
                    t = loadstep_sheet.cell_value(p, q)
                    if t == 1:
                        x.append(1)
                    else:
                        x.append(0)
                loadstep_matrix.append(x)
                ...
                if (1 in pretension):
                    ...
                    if 'Pretension' in ls_list:
                        ...
                        if (pretsheet.cell_value(pretsheet.nrows - 1, 1) == 'Pretension'):
                            ...
                            self.errorLabel.setText(
                                self.errorLabel.text() + "Warning: No Pretension found, will use 0 as Pretension ID.\n")
                            pretloadid = 0
                        ...
                        new_tcl.write('*attributeupdateentity loadsteps ' + pret_ID + ' 4145 1 1 0 loadcols ' + str(
                            int(pretspcid)) + '\n')
                        if lgdisp == 0:
                            new_tcl.write('*attributeupdateentity loadsteps ' + pret_ID + ' 4187 1 1 0 loadcols ' + str(
                                global_dict['nlparmID']) + '\n')
                        else:
                            new_tcl.write('*attributeupdateentity loadsteps ' + pret_ID + ' 9931 1 1 0 loadcols ' + str(
                                global_dict['nlparmID']) + '\n')
                        ...
                        new_tcl.write('*attributeupdateentity loadsteps ' + pret_ID + ' 2159 1 1 0 loadcols ' + str(
                            int(pretloadid)) + '\n')
                ...
                if (loadstep_sheet.cell_value(i + 3, 1) == 'linear static'):
                    ...
                    if (loadstep_matrix[i].count(1) == 1):
                        ...
                        if not (str(loadstep_sheet.cell_value(1, loadstep_matrix[i].index(1) + 3)) in spcname):
                            ...
                            # new_tcl.write(line)
                            self.errorLabel.setText(
                                self.errorLabel.text() + 'Error: Only Load is selected in step ' + str(
            ...
            if (errorCount == 0):
                ...
                # new_tcl.write('if [ catch {*cardcreate "GLOBAL_OUTPUT_REQUEST"} ] {\n')
                # new_tcl.write('set answer [tk_messageBox -title "Warning" -message "GLOBAL_OUTPUT_REQUEST card not created!" -type ok]\n')
                # new_tcl.write('}\n')
                ...
                # new_tcl.write('*startnotehistorystate {Modified control card}\n')
                # new_tcl.write('if [ catch {*cardcreate "OUTPUT"} ] {\n')
                # new_tcl.write(
                #     'set answer [tk_messageBox -title "Warning" -message "OUTPUT card not created!" -type ok]\n')
                # new_tcl.write('}\n')
                # new_tcl.write('*startnotehistorystate {Attached attributes to card}\n')
            ...
            if errorCount == 0:
                ...
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Information)
                msg.setText("TCL has been successfully generated!")
