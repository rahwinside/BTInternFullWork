...
class UX(QDialog, Ui_load):
    ...
    def gen_excel(self):
        global hyper_path, max_LS, nlparm, nlparm_id, ls_list, pret_ls_id, ls_id_list, lc_id, collector_name_list, max_SPC_add_count
        nlparm = True
        nlparm_id = 10000
        try:
            try:
            ...
            for item in entity_name:
                collector_name_list.append(item)
            ...
            max_SPC_add_count = 1
            for item in collector_name_list:
            ...
            ldcase = wkbk.add_worksheet('LOADADD')
            ldstep = wkbk.add_worksheet('LOADSTEP')
            ...
            if "Pretension" in entity_name:
                ldcase.write(0, i + 2, "Pretension")
                ldcase.write(2, i + 2, int(temp_dict["Pretension"]))
            ...
            for j in range(0, 16370):
                ldstep.write(0, j + i + 4,
                             '=IF(LOADADD!A' + str(j + 4) + '="", "", LOADADD!A' + str(j + 4) + ')')
            wkbk.close()
    ...
    def spcAdd(self, wkbook, id):
        global hyper_path, spcList, selSPCNoDupl, spcAddDict, l1, ls_list, ls_id_list, max_SPC_add_count
        spcList = []
        spcAddDict = []
        selSPCNoDupl = []
        ...
        i = 4
        spcName:str = sheet.cell_value(0, i)
        spcList.append(spcName)
        while spcName.startswith('SPC'):
            i += 1
            spcName: str = sheet.cell_value(0, i)
            if(spcName.startswith('SPC')):
                spcList.append(spcName)

        selSPCArray = []

        for i in range(2, nLoadsteps):
            checked = []
            for j in range(4, len(spcList)+4):
        ...
        for item in selSPCNoDupl:
            ...
            for i in range(0, len(spcList)):
                if item[i] == 1:
                    text += str(int(sheet.cell_value(1, i+4))) + ' '
            text += '\n'
            new_tcl.write(text)
    ...
    def main(self):
        ...
        # zerobook = openpyxl.load_workbook(wkbook)
        # sheetN = zerobook['LoadcaseSheet']
        #
        # #zero empty spaces
        # i = 1
        #
        # for z in range(rows):
        #     while i <= nlcount:
        #         print(sheetN.cell(row = 4+z, column = i+2).value)
        #         if(sheetN.cell(row = 4+z, column = i+2).value == None):
        #             sheetN.cell(row = 4+z, column = i+2).value = 0
        #         i += 1
        #
        # zerobook.save(wkbook)
        # zerobook.close()
    ...
    def loadcase(self, wkbook):
        ...
        try:
            ...
            for i in range(2, nlcount + 2):
                if((sheet.cell_value(0, i), int(sheet.cell_value(2, i))) not in l1):
            ...
            for i in range(0, nlcase):
                ...
                k = nlcount - LCount[i].count(0) - LCount[i].count(str(0.0)) # tcl: k is no. of non-zero scaling factors in a loadcase (non-zero cells in a excel row)
    ...
    def loadstep(self, wkbook):
        global global_dict, max_LS, ls_list, pret_ls_id
        ...
        try:
            ...
            loadstep_matrix = [[0 if loadstep_sheet.cell_value(i + 2, j + 4) == '' else int(loadstep_sheet.cell_value(i + 2, j + 4)) for j in range(nLoadcombs)] for i in
                               range(nLoadsteps)]
                        ...
                        try:
                            ...
                            if spcArray.count(1) == 1:
                                line = '*attributeupdateentity loadsteps ' + id + ' 4145 1 1 0 loadcols ' + str(
                                    global_dict[loadstep_sheet.cell_value(0, loadstep_matrix[i].index(1) + 4)]) + '\n'
                                new_tcl.write(line)
                                print(loadstep_sheet.cell_value(0, len(loadstep_matrix[i]) -
                                                                          loadstep_matrix[i][::-1].index(
                                                                              1) - 1 + 4))
                                line = '*attributeupdateentity loadsteps ' + id + ' 4147 1 1 0 loadcols ' + str(
                                    global_dict[loadstep_sheet.cell_value(0, len(loadstep_matrix[i]) -
